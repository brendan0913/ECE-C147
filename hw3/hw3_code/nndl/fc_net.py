...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    scores, scores_cached = affine_forward(out2, w2, b2) 
    ...
    dx2, dw2, db2 = affine_backward(dx, scores_cached)
    da1 = relu_backward(dx2, c2)
    dx1, dw1, db1 = affine_backward(da1, c1)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for layer in range(self.num_layers):
      self.params['W' + str(layer + 1)] = weight_scale * np.random.randn(dims[layer], dims[layer+1])
